<template>
  <div class="app-container">
    <div class="filter-container">
      <el-button class="filter-item" style="margin-left: 10px;" type="primary" icon="el-icon-edit" @click="handleCreate">{{ $t('table.add') }}</el-button>
    </div>

    <el-table
      v-loading="listLoading"
      :key="tableKey"
      :data="list"
      border
      fit
      highlight-current-row
      style="width: 100%;"
      @sort-change="sortChange">
      <el-table-column :label="$t('table.id')" prop="id" sortable="custom" align="center" width="65">
        <template slot-scope="scope">
          <span>{{ scope.row.id }}</span>
        </template>
      </el-table-column>
      <el-table-column :label="$t('table.game_name_vi')" min-width="150px">
        <template slot-scope="scope">
          <span class="link-type">{{ scope.row.name_vi }}</span>
        </template>
      </el-table-column>
      <el-table-column :label="$t('table.game_name_en')" min-width="150px">
        <template slot-scope="scope">
          <span class="link-type">{{ scope.row.name_en }}</span>
        </template>
      </el-table-column>
      <el-table-column :label="$t('table.photo')" min-width="150px">
        <template slot-scope="scope">
          <img :src="imgUrl(scope.row.photo)">
        </template>
      </el-table-column>
      <el-table-column :label="$t('table.actions')" align="center" width="230" class-name="small-padding fixed-width">
        <template slot-scope="scope">
          <el-button type="primary" size="mini" @click="handleUpdate(scope.row)">{{ $t('table.edit') }}</el-button>
          <el-button v-if="scope.row.status!='deleted'" size="mini" type="danger" @click="handleDelete(scope.row)">{{ $t('table.delete') }}
          </el-button>
        </template>
      </el-table-column>
    </el-table>

    <pagination v-show="total>0" :total="total" :page.sync="listQuery.page" :limit.sync="listQuery.limit" @pagination="getList" />

    <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible">
      <el-form ref="dataForm" :rules="rules" :model="temp"
        label-position="left"
        label-width="70px"
        style="width: 400px; margin-left:50px;">
        <el-form-item label-width="140px" :label="$t('table.game_name_vi')" prop="name_vi">
          <el-input v-model="temp.name_vi"/>
        </el-form-item>
        <el-form-item label-width="140px" :label="$t('table.game_name_en')" prop="name_en">
          <el-input v-model="temp.name_en"/>
        </el-form-item>
        <el-form-item label-width="140px" :label="$t('table.level_id')" prop="level_id">
          <el-select class="filter-item" filterable
            v-model="temp.level_id"
            placeholder="Hãy chọn level">
            <el-option v-for="item in levels"
              :key="item.id"
              :label="item.name_vi"
              :value="item.id"/>
          </el-select>
        </el-form-item>
        <el-form-item v-if="dialogStatus==='create'" label-width="140px" :label="$t('table.upload')" prop="photo">
          <single-upload ref="upload" :lable="'Upload ảnh'"></single-upload>
        </el-form-item>
      </el-form>
      <div slot="footer" class="dialog-footer">
        <el-button @click="dialogFormVisible = false">{{ $t('table.cancel') }}</el-button>
        <el-button type="primary" @click="dialogStatus==='create' ? createData(): updateData()">{{ $t('table.confirm') }}</el-button>
      </div>
    </el-dialog>
  </div>
</template>

<script>
import { fetchList, createGame, deleteGame } from '@/api/game_contect';
import Pagination from '@core/components/Pagination';
import SingleUpload from '@/components/Upload/SingleUpload';
import _ from 'lodash';
import { getLevels } from '@/api/words_level';

export default {
  name: 'Game',
  components: {
    Pagination,
    SingleUpload,
  },
  data() {
    return {
      tableKey: 0,
      list: [],
      total: 0,
      listLoading: true,
      listQuery: {
        page: 1,
        limit: 20,
        importance: undefined,
        title: undefined,
        type: undefined,
        sort: '+id',
      },
      temp: {
        id: undefined,
        name_vi: '',
        name_en: '',
        level_id: '',
      },
      dialogFormVisible: false,
      dialogStatus: '',
      textMap: {
        update: 'Cập nhập level',
        create: 'Thêm mới level',
      },
      dialogPvVisible: false,
      rules: {
        name_vi: [{ required: true, message: 'Tên trò chơi không được trống', trigger: 'change' }],
        name_en: [{ required: true, message: 'Tên trò chơi(en) không được trống', trigger: 'change' }],
      },
      formData: new FormData(),
      photo_options: {
        height: '',
        width: '',
      },
      baseLevels: [1, 2, 3],
      levels: {},
    };
  },
  created() {
    this.getList();
    getLevels().then(res => {
      if (res.success) {
        this.levels = _.filter(res.data, level => {
          return this.baseLevels.findIndex(i => i === level.id) >= 0;
        });
      }
    });
  },
  mounted() {
  },
  methods: {
    getList(query = {}) {
      this.listLoading = true;
      if (_.size(query)) {
        this.listQuery.page = query.page;
        this.listQuery.limit = query.limit;
      }
      fetchList(this.listQuery).then(response => {
        this.list = response.data.data;
        this.total = response.data.total;
        this.listLoading = false;
      });
    },
    handleFilter() {
      this.listQuery.page = 1;
      this.getList();
    },
    sortChange(data) {
      const { prop, order } = data;
      if (prop === 'id') {
        this.sortByID(order);
      }
    },
    sortByID(order) {
      if (order === 'ascending') {
        this.listQuery.sort = '+id';
      } else {
        this.listQuery.sort = '-id';
      }
      this.handleFilter();
    },
    resetTemp() {
      this.temp = {
        id: undefined,
        name_vi: '',
        name_en: '',
      };
    },
    handleCreate() {
      this.resetTemp();
      this.dialogStatus = 'create';
      this.dialogFormVisible = true;
      this.$nextTick(() => {
        this.$refs['dataForm'].clearValidate();
      });
    },
    createData() {
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          const self = this;
          this.setPhotoOptions(function() {
            self.photo_options.height = this.height;
            self.photo_options.width = this.width;
            self.formData.append('photo_options', JSON.stringify(self.photo_options));
            self.formData.append('height', this.height);
            self.formData.append('width', this.width);
            self.handleCreateData();
          });
        }
      });
    },
    handleCreateData() {
      this.formData.append('file', this.$refs.upload.file);
      this.appendData();
      createGame(this.formData).then((res) => {
        if (res.success) {
          this.getList();
          this.dialogFormVisible = false;
          this.$notify({
            title: 'Success',
            message: 'Thêm mới thành công !!!',
            type: 'success',
            duration: 2000,
          });
        }
      });
    },
    appendData() {
      _.forEach(this.temp, (t, i) => {
        this.formData.append(`${i}`, t);
      });
    },
    setPhotoOptions(callback) {
      const img = new Image();
      img.src = URL.createObjectURL(this.$refs.upload.file);
      img.onload = callback;
    },
    imgUrl(image) {
      const i = image || 'no-image.jpg';
      return `/upload/image/${i}`;
    },
    handleUpdate(row) {
      this.$router.push({
        name: 'update-game',
        query: { id: row.id },
      });
    },
    handleDelete(row) {
      if (confirm('Bạn có chắc chắn muốn xóa dòng này ?')) {
        deleteGame({ id: row.id }).then((res) => {
          if (res.success) {
            const index = this.list.indexOf(row);
            this.list.splice(index, 1);
            this.dialogFormVisible = false;
            this.$notify({
              title: 'Success',
              message: 'Xóa level thành công !!!',
              type: 'success',
              duration: 2000,
            });
          }
        });
      }
    },
  },
};
</script>

<style scoped>
</style>
